...
import { OperationDirection, NetworkType, UserRole } from '@prisma/client'
...
export function EditRequestModal({ requestId, open, onOpenChange, onRequestUpdated }: EditRequestModalProps) {
  const { user } = useAuth()
  ...
  const { execute: fetchRequest } = useApi<RequestData>()
  const { execute: updateRequest, loading: updating } = useApi()
  ...
  // Заполняем форму при получении данных заявки
  useEffect(() => {
    if (request) {
      populateForm()
    }
  ...
  const populateForm = () => {
    ...
    setFormData({
      ...
      fromCurrency: request.finance?.fromCurrency || '',
      toCurrency: request.finance?.toCurrency || '',
      expectedAmountFrom: request.finance?.expectedAmountFrom?.toString() || '',
      walletAddress: request.requisites?.walletAddress || '',
      cardNumber: request.requisites?.cardNumber || '',
      bankName: request.requisites?.bankName || '',
    })
  }
  ...
  const loadData = async () => {
    ...
    const [requestResult, clientsResult, officesResult, currenciesResult] = await Promise.all([
      fetchRequest(`/api/requests/${requestId}`),
  ...
  // Показываем загрузку если модальное окно открыто но данные еще не загружены
  const isModalReady = request && clients.length > 0
        ...
        <DialogHeader>
          {isModalReady ? (
            <div className="flex items-center justify-between">
              <div>
                <DialogTitle className="text-xl font-semibold">Редактирование заявки {request.requestId}</DialogTitle>
                <DialogDescription>
                  Измените информацию о заявке
                </DialogDescription>
              </div>
            ...
            <div>
              <DialogTitle className="text-xl font-semibold">Редактирование заявки</DialogTitle>
              <DialogDescription>
            ...
            <div className="space-y-6 animate-in slide-in-from-right-5 duration-300">
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>Изменение клиента и типа операции</CardDescription>
                </CardHeader>
            ...
            <div className="space-y-6 animate-in slide-in-from-right-5 duration-300">
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>Изменение валют и суммы операции</CardDescription>
                </CardHeader>
                <CardContent className="space-y-4">
                  <div className="space-y-2">
